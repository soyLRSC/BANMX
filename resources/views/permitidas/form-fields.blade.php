...
<div class="form-floating mb-3">
    <input type="time" class="form-control" id="hora" name="hora"
        value="{{ old('hora', $permitida->hora ?? '') }}" style="background-color: #E6E6FA;" required>
...
<div class="form-floating mb-3">
    <input type="date" class="form-control" id="fecha" name="fecha"
        value="{{ old('fecha', $permitida->fecha ?? date('Y-m-d')) }}" style="background-color: #E6E6FA;" required>
    ...
    <select class="form-select" id="encargado" name="encargado" style="background-color: #E6E6FA;" required>
        ...
        @foreach ([
            ...
            <option value="{{ $encargado }}"
                {{ old('encargado', $permitida->encargado ?? '') == $encargado ? 'selected' : '' }}>
                {{ $encargado }}
            </option>
        @endforeach
    </select>
...
<div class="form-floating mb-3">
    <select class="form-select" id="motivo" name="motivo" style="background-color: #E6E6FA;" required>
        ...
        <option value="fumar" {{ old('motivo', $permitido->motivo ?? '') == 'Fumar' ? 'selected' : '' }}>Sale a fumar</option>
        <option value="SanitaRIO" {{ old('motivo', $permitido->motivo ?? '') == 'Sanitario' ? 'selected' : '' }}>Sanitario</option>
    </select>
